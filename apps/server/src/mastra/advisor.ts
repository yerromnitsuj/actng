...
- set_trend_selections records the judgment (and the target cost-level year). These arm the expected-loss-ratio machinery; they do not change current method results, so no rerun is needed. Rates in set_trend_selections are DECIMALS (0.065); analyze_trends reports PERCENT (6.5) - convert. clearTargetYear true restores the floating latest-origin-year target.
