...
// No rscriptAvailable() gate needed here (contrast app.test.ts and
// example.test.ts): the Rscript preflight only runs on CLI boot via
// argv[1], and this file never calls computeWithEngine, so it runs fine on
// contributors' machines without R installed.
