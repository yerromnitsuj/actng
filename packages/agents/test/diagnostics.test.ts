...
describe("diagnostic trusted-catalog tool", () => {
  ...
  it("runs an exact, sorted selection and returns only selected identity keys", async () => {
    ...
    const tool = createDiagnosticSelectionTool({
      ...
      presets: [
        preset({
          execute: async ({ tenant, instanceIds }) => {
            expect(tenant).toBe("tenant-7");
    ...
    expect(Object.keys(result.calculationFingerprints)).toEqual([
      ids[1],
      ids[0],
    ]);
  ...
  it("rejects unauthenticated, cross-definition, wrong-preset, and cached-superset runs", async () => {
    ...
    for (const execute of cases) {
      const tool = createDiagnosticSelectionTool({
        definition: compiled,
        presets: [preset({ execute: execute as never })],
      });
      await expect(
        tool.execute(
  ...
  it("validates the complete host catalog atomically", () => {
    const bad = [
      ...
      () =>
        createDiagnosticSelectionTool({
          definition: compiled,
          presets: [preset({ allowedInstanceIds: [ids[0], ids[0]] })],
        }),
      () =>
        createDiagnosticSelectionTool({
          definition: compiled,
          presets: [preset({ allowedInstanceIds: ["unknown"] })],
        }),
      () =>
        createDiagnosticSelectionTool({
          definition: compiled,
          presets: [preset({ definitionIntegrity: "wrong" })],
        }),
      () =>
        createDiagnosticSelectionTool({
          definition: compiled,
          presets: [preset({ execute: null as never })],
        }),
    ];
