...
function entry(
  overrides: Partial<NewAssumptionEntry> = {},
): NewAssumptionEntry {
  return {
...
function expectComplianceError(
  fn: () => unknown,
  code: string,
): ComplianceError {
  let thrown: unknown;
...
describe("recordAssumption", () => {
  it("assigns seq = entries.length + 1 and preserves the entry fields", () => {
    ...
    ledger = recordAssumption(
      ...
      entry({
        field: "b",
        actor: "actuary",
        rationale: "industry benchmark",
        source: "Friedland Table 15",
      }),
    );
  ...
  it("snapshots and deeply freezes nested caller-owned values", () => {
    const value = { nested: { selection: 1 } };
    const ledger = recordAssumption(createLedger(), entry({ value }));
    value.nested.selection = 99;
    expect(ledger.entries[0]!.value).toEqual({ nested: { selection: 1 } });
    expect(
      Object.isFrozen((ledger.entries[0]!.value as { nested: object }).nested),
    ).toBe(true);
  ...
  it("allows actor default without a rationale, and judgment with one", () => {
    ...
    ledger = recordAssumption(
      ledger,
      entry({
        field: "bf.aprioriLossRatio",
        actor: "agent",
        rationale: "fit to latest three diagonals",
      }),
    );
...
describe("judgmentEntries", () => {
  it("returns only entries where actor is not default, in ledger order", () => {
    ...
    ledger = recordAssumption(
      ledger,
      entry({ field: "b", actor: "actuary", rationale: "r1" }),
    );
    ledger = recordAssumption(
      ledger,
      entry({ field: "c", actor: "agent", rationale: "r2" }),
    );
...
describe("changedAssumptions", () => {
  function ledgerOf(...entries: NewAssumptionEntry[]): AssumptionLedger {
    return entries.reduce(
      (ledger, e) => recordAssumption(ledger, e),
      createLedger(),
    );
  }
  ...
  it("reports added, removed, and value-changed fields, sorted by field", () => {
    ...
    expect(report.changed).toEqual([
      { field: "tail", priorValue: 1.05, currentValue: 1.1 },
    ]);
  });
  ...
  it("uses the LATEST entry per field on each side", () => {
    ...
    expect(changedAssumptions(prior, backToStart)).toEqual({
      added: [],
      removed: [],
      changed: [],
    });
  ...
  it("compares values canonically: key order and -0 never cause a false change", () => {
    ...
    expect(changedAssumptions(prior, current)).toEqual({
      added: [],
      removed: [],
      changed: [],
    });
  ...
  it("still flags genuinely different structured values", () => {
    ...
    const current = ledgerOf(
      entry({ field: "curve", value: { a: 1, b: [2, 1] } }),
    );
    const report = changedAssumptions(prior, current);
  ...
  it("sorts every output list by field for deterministic disclosure rendering", () => {
    const prior = ledgerOf(
      entry({ field: "z", value: 1 }),
      entry({ field: "a", value: 1 }),
    );
    const current = ledgerOf(
      entry({ field: "m", value: 1 }),
      entry({ field: "b", value: 1 }),
    );
    const report = changedAssumptions(prior, current);
  ...
  it("returns an empty report for identical ledgers", () => {
    ...
    expect(changedAssumptions(ledger, ledger)).toEqual({
      added: [],
      removed: [],
      changed: [],
    });
