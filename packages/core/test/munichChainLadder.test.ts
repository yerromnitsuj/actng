...
const qmPublished = sourceExpected<
  typeof import("./fixtures/quargMack2004.js").qmPublished
>("quarg-mack-2004", "qmPublished");
...
import { qmIncurred, qmPaid } from "./fixtures/quargMack2004.js";
...
describe("Munich chain ladder: Quarg-Mack (2004) printed parameters", () => {
  it("reproduces fhat^P and fhat^I to the printed 3 decimals", () => {
    ...
    qmPublished.paidFactors.forEach((published, s) => {
      expect(result.paidFactors[s]!).toBeCloseTo(
        published,
        sourceTolerance("quarg-mack-2004", "decimalPlaces:3"),
      );
    });
    qmPublished.incurredFactors.forEach((published, s) => {
      expect(result.incurredFactors[s]!).toBeCloseTo(
        published,
        sourceTolerance("quarg-mack-2004", "decimalPlaces:3"),
      );
    });
  ...
  it("reproduces sigmahat^P and sigmahat^I (Mack denominators, pairs - 1) to 3 decimals", () => {
    qmPublished.sigmaPaid.forEach((published, s) => {
      expect(result.sigmaPaid[s]!).toBeCloseTo(
        published,
        sourceTolerance("quarg-mack-2004", "decimalPlaces:3"),
      );
    });
    qmPublished.sigmaIncurred.forEach((published, s) => {
      expect(result.sigmaIncurred[s]!).toBeCloseTo(
        published,
        sourceTolerance("quarg-mack-2004", "decimalPlaces:3"),
      );
    });
    // The final column is the paper's manual 0.100, surfaced in warnings.
    expect(result.sigmaPaid[5]!).toBeCloseTo(
      sourceExpected<number>("quarg-mack-2004", "literal:0.1"),
      sourceTolerance("quarg-mack-2004", "decimalPlaces:12"),
    );
    expect(result.sigmaIncurred[5]!).toBeCloseTo(
      sourceExpected<number>("quarg-mack-2004", "literal:0.1"),
      sourceTolerance("quarg-mack-2004", "decimalPlaces:12"),
    );
  ...
  it("reproduces the incurred-weighted qhat_s row to the printed 0.1%", () => {
    ...
    qmPublished.qRatios.forEach((published, s) => {
      expect(result.qRatios[s]!).toBeCloseTo(
        published,
        sourceTolerance("quarg-mack-2004", "decimalPlaces:3"),
      );
    });
  ...
  it("reproduces rhohat^P and rhohat^I (ratio-series Mack variances, cells - 1) to 3 decimals", () => {
    qmPublished.rhoPaid.forEach((published, s) => {
      expect(result.rhoPaid[s]!).toBeCloseTo(
        published,
        sourceTolerance("quarg-mack-2004", "decimalPlaces:3"),
      );
    });
    qmPublished.rhoIncurred.forEach((published, s) => {
      expect(result.rhoIncurred[s]!).toBeCloseTo(
        published,
        sourceTolerance("quarg-mack-2004", "decimalPlaces:3"),
      );
    });
  ...
  it("reproduces the paper's printed residual spot values", () => {
    ...
    expect(result.residuals.paidRatio[6]![0]!).toBeCloseTo(
      qmPublished.residualPins.paidRatioAY7,
      sourceTolerance("quarg-mack-2004", "decimalPlaces:3"),
    );
  ...
  it("reproduces the through-origin regression lambdas within 0.01", () => {
    expect(Math.abs(result.lambdaPaid - qmPublished.lambdaPaid)).toBeLessThan(
      sourceTolerance("quarg-mack-2004", "toBeLessThan:0.01"),
    );
    expect(
      Math.abs(result.lambdaIncurred - qmPublished.lambdaIncurred),
    ).toBeLessThan(sourceTolerance("quarg-mack-2004", "toBeLessThan:0.01"));
  });
});
...
describe("Munich chain ladder: Quarg-Mack (2004) printed projections", () => {
  it("reproduces the worked first step for accident year 7 (the simultaneous recursion)", () => {
    ...
    expect(
      Math.abs(paid24 - qmPublished.workedStepAY7.paidAge24),
    ).toBeLessThanOrEqual(
      sourceTolerance("quarg-mack-2004", "toBeLessThanOrEqual:1"),
    );
    expect(
      Math.abs(incurred24 - qmPublished.workedStepAY7.incurredAge24),
    ).toBeLessThanOrEqual(
      sourceTolerance("quarg-mack-2004", "toBeLessThanOrEqual:1"),
    );
    // Implied MCL first factors 2.768 / 1.559, and Q_{7,2} = 72.3%.
    expect(paid24 / 2044).toBeCloseTo(
      qmPublished.workedStepAY7.mclPaidFactor,
      sourceTolerance("quarg-mack-2004", "decimalPlaces:3"),
    );
    expect(incurred24 / 5022).toBeCloseTo(
      qmPublished.workedStepAY7.mclIncurredFactor,
      sourceTolerance("quarg-mack-2004", "decimalPlaces:3"),
    );
    expect(paid24 / incurred24).toBeCloseTo(
      qmPublished.workedStepAY7.ratioAge24,
      sourceTolerance("quarg-mack-2004", "decimalPlaces:3"),
    );
  });
  ...
  it("reproduces every printed projected cell of both result quadrangles within 1 absolute", () => {
    for (let i = 0; i < 7; i++) {
      ...
      qmPublished.mclProjectedIncurred[i]!.forEach((published, k) => {
        expect(
          Math.abs(
            result.projectedIncurred[i]![firstProjected + k]! - published,
          ),
        ).toBeLessThanOrEqual(
          sourceTolerance("quarg-mack-2004", "toBeLessThanOrEqual:1"),
        );
  ...
  it("reproduces the MCL ultimates per accident year within 1 absolute (well under 0.5%)", () => {
    ...
    qmPublished.mclPaidUltimates.forEach((published, i) => {
      expect(
        Math.abs(result.rows[i]!.paidUltimate - published),
      ).toBeLessThanOrEqual(
        sourceTolerance("quarg-mack-2004", "toBeLessThanOrEqual:1"),
      );
      expect(
        Math.abs(result.rows[i]!.paidUltimate - published) / published,
      ).toBeLessThan(sourceTolerance("quarg-mack-2004", "toBeLessThan:0.005"));
    });
    qmPublished.mclIncurredUltimates.forEach((published, i) => {
      expect(
        Math.abs(result.rows[i]!.incurredUltimate - published),
      ).toBeLessThanOrEqual(
        sourceTolerance("quarg-mack-2004", "toBeLessThanOrEqual:1"),
      );
    });
  ...
  it("matches Figure 16's SCL anchors: AY7 paid falls ~27% short, AY6 exceeds by ~10%", () => {
    expect(result.rows[6]!.sclFinalRatio!).toBeGreaterThan(
      sourceTolerance("quarg-mack-2004", "toBeGreaterThan:0.7"),
    );
    expect(result.rows[6]!.sclFinalRatio!).toBeLessThan(
      sourceTolerance("quarg-mack-2004", "toBeLessThan:0.76"),
    );
    expect(result.rows[5]!.sclFinalRatio!).toBeGreaterThan(
      sourceTolerance("quarg-mack-2004", "toBeGreaterThan:1.08"),
    );
    expect(result.rows[5]!.sclFinalRatio!).toBeLessThan(
      sourceTolerance("quarg-mack-2004", "toBeLessThan:1.12"),
    );
    // MCL ultimate P/I about 97-100% in all years.
    for (const row of result.rows) {
      expect(row.finalRatio!).toBeGreaterThan(
        sourceTolerance("quarg-mack-2004", "toBeGreaterThan:0.965"),
      );
      expect(row.finalRatio!).toBeLessThan(
        sourceTolerance("quarg-mack-2004", "toBeLessThan:1.005"),
      );
    }
  });
  ...
  it("closes the paid/incurred gap: |MCL P/I - 1| <= |SCL P/I - 1| per year (documented AY4 slack)", () => {
    ...
    for (const row of result.rows) {
      ...
      expect(mclGap).toBeLessThanOrEqual(
        sclGap +
          sourceTolerance("quarg-mack-2004", "toBeLessThanOrEqual:0.001"),
      );
    ...
    expect(
      result.totals.paidUltimate / result.totals.incurredUltimate,
    ).toBeGreaterThan(
      sourceTolerance("quarg-mack-2004", "toBeGreaterThan:0.98"),
    );
    expect(
      result.totals.paidUltimate / result.totals.incurredUltimate,
    ).toBeLessThan(sourceTolerance("quarg-mack-2004", "toBeLessThan:1.005"));
  });
  ...
  it("falls back to Mack's sigma extrapolation (with a warning) when no manual last sigma is given", () => {
    ...
    expect(fallback.lambdaPaid).toBeCloseTo(
      result.lambdaPaid,
      sourceTolerance("quarg-mack-2004", "decimalPlaces:12"),
    );
    expect(fallback.sigmaPaid[4]!).toBeCloseTo(
      result.sigmaPaid[4]!,
      sourceTolerance("quarg-mack-2004", "decimalPlaces:12"),
    );
    // Only the final development step differs, so earlier projections agree.
    expect(fallback.projectedPaid[6]![1]!).toBeCloseTo(
      result.projectedPaid[6]![1]!,
      sourceTolerance("quarg-mack-2004", "decimalPlaces:9"),
    );
  });
});
...
describe("Munich chain ladder: degenerate inputs", () => {
  ...
  it("throws SHAPE when the triangles' origins or ages differ", () => {
    ...
    expectCode("SHAPE", () =>
      runMunichChainLadder(qmPaid, {
        ...qmIncurred,
        ages: [12, 24, 36, 48, 60, 72, 96],
      }),
    );
    expectCode("SHAPE", () =>
      runMunichChainLadder(qmPaid, {
        ...qmIncurred,
        values: qmIncurred.values.map((row, i) =>
          i === 3 ? row.slice(0, 5) : row,
        ),
      }),
    );
  });
  ...
  it("throws TOO_SMALL when no development column has two paired factors", () => {
    ...
    expectCode("TOO_SMALL", () =>
      runMunichChainLadder(smallPaid, smallIncurred),
    );
  ...
  it("collapses to the separate chain ladder (lambda = 0, with a warning) when P/I is in lockstep", () => {
    ...
    const incurred: Triangle = {
      ...
      ages: paid.ages,
      values: paid.values.map((row) =>
        row.map((v) => (v === null ? null : v * 2)),
      ),
    };
    ...
    for (const row of collapsed.rows) {
      expect(row.paidUltimate).toBeCloseTo(
        row.sclPaidUltimate,
        sourceTolerance("quarg-mack-2004", "decimalPlaces:9"),
      );
      expect(row.incurredUltimate).toBeCloseTo(
        row.sclIncurredUltimate,
        sourceTolerance("quarg-mack-2004", "decimalPlaces:9"),
      );
    }
  });
  ...
  it("skips origins with no observations in one triangle, with a warning", () => {
    const paid: Triangle = {
      ...qmPaid,
      values: qmPaid.values.map((row, i) =>
        i === 6 ? row.map(() => null) : row,
      ),
    };
