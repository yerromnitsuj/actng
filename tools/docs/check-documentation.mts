...
const mode = process.argv.find((arg) => arg.startsWith("--mode="))?.slice(7) ?? "base";
const docs = execFileSync("git", ["ls-files", "--cached", "--others", "--exclude-standard", "--", "*.md", "*.mdx"], { cwd: root, encoding: "utf8" }).trim().split("\n").filter(Boolean).sort();
...
function walk(node: MdNode, visit: (node: MdNode) => void): void { visit(node); for (const child of node.children ?? []) walk(child, visit); }

function ownership(path: string): string[] {
  const packageMatch = path.match(/^packages\/([^/]+)\//); if (packageMatch && SDK_PACKAGES.includes(packageMatch[1]!)) return [packageMatch[1]!];
  if (path.startsWith("interop/") || path.startsWith("schema/interchange/") || path.startsWith("tools/interop/")) return ["interchange"];
...
interface Fence { path: string; heading: string; language: string; ordinal: number; sha256: string; classification: string; reason: string }
function parsed(path: string, text: string): { fences: Fence[]; sources: Map<number, string>; links: string[] } {
  const tree = fromMarkdown(text) as MdNode;
  const definitions = new Map<string, string>();
  walk(tree, (node) => { if (node.type === "definition" && node.identifier && node.url) definitions.set(node.identifier.toLowerCase(), node.url); });
  const result: Fence[] = []; const sources = new Map<number, string>(); const links: string[] = [];
  let heading = "(document)"; let ordinal = 0;
  walk(tree, (node) => {
    if (node.type === "heading") heading = nodeText(node);
    if (node.type === "link" || node.type === "image") { if (node.url) links.push(node.url); }
    if (node.type === "linkReference" || node.type === "imageReference") { const url = definitions.get(node.identifier?.toLowerCase() ?? ""); if (url) links.push(url); }
    if (node.type !== "code") return;
    ordinal += 1; const language = (node.lang ?? "text").toLowerCase(); const source = node.value ?? "";
    const isPublic = PUBLIC_LANGUAGES.has(language);
    result.push({ path, heading, language, ordinal, sha256: hash(source), classification: isPublic ? "reviewed-public" : "reviewed-illustrative", reason: isPublic ? "Executable/configuration example syntax-checked here and behavior-owned by its active document or linked test/example." : "Non-executable output, diagram, or prose example." });
...
function caseSensitiveExists(path: string): boolean {
  const rel = relative(root, path); if (rel === "" || rel === ".") return true;
  if (rel.startsWith(`..${sep}`) || rel === "..") return false;
  let current = root;
  for (const part of rel.split(sep)) { if (!existsSync(current) || !statSync(current).isDirectory() || !readdirSync(current).includes(part)) return false; current = resolve(current, part); }
...
const inventory = JSON.parse(readFileSync(inventoryPath, "utf8")) as { schemaVersion: number; documents: Record<string, { classification: string; reason: string; packages: string[] }> };
const manifest = JSON.parse(readFileSync(snippetsPath, "utf8")) as { schemaVersion: number; fences: Fence[] };
const failures: string[] = [];
if (inventory.schemaVersion !== 1 || manifest.schemaVersion !== 1) failures.push("documentation manifests require schemaVersion 1");
if (JSON.stringify(Object.keys(inventory.documents).sort()) !== JSON.stringify(docs)) failures.push("documentation inventory does not exactly match tracked and nonignored Markdown/MDX files");
...
for (const path of docs) {
  ...
  for (const destination of parsedByPath.get(path)!.links) {
    ...
    let decoded: string; try { decoded = decodeURIComponent(rawFile!); } catch { failures.push(`${path}: malformed percent-encoding in ${destination}`); continue; }
    const target = resolve(dirname(resolve(root, path)), decoded);
    if (!caseSensitiveExists(target)) { failures.push(`${path}: missing or case-mismatched link target ${destination}`); continue; }
    if (rawFragment && /\.mdx?$/.test(rawFile!)) {
      const targetTree = fromMarkdown(readFileSync(target, "utf8")) as MdNode; const headings: string[] = [];
      walk(targetTree, (node) => { if (node.type === "heading") headings.push(slug(nodeText(node))); });
      if (!headings.includes(decodeURIComponent(rawFragment).toLowerCase())) failures.push(`${path}: missing anchor ${destination}`);
    }
  }
}

for (const [path, required] of Object.entries({
  "README.md": ["docs/reference/diagnostic-formulas.md", "docs/migrations/0.6-generalized-diagnostics.md"],
  "docs/README.md": ["reference/diagnostic-formulas.md", "migrations/0.6-generalized-diagnostics.md"],
  "CHANGELOG.md": ["docs/reference/diagnostic-formulas.md", "docs/migrations/0.6-generalized-diagnostics.md"],
})) for (const destination of required) if (!readFileSync(resolve(root, path), "utf8").includes(destination)) failures.push(`${path}: missing required inbound link to ${destination}`);
for (const packageName of SDK_PACKAGES) for (const required of ["diagnostic-formulas.md", "0.6-generalized-diagnostics.md"]) {
  const readme = readFileSync(resolve(root, `packages/${packageName}/README.md`), "utf8");
  if (!readme.includes("https://github.com/yerromnitsuj/actng/blob/v0.6.0/docs/") || !readme.includes(required)) failures.push(`packages/${packageName}/README.md: missing tagged ${required} link`);
}

function publicExports(): Map<string, Set<string>> {
  const entrypoints = SDK_PACKAGES.map((name) => resolve(root, `packages/${name}/dist/index.d.ts`));
  const program = ts.createProgram(entrypoints, { module: ts.ModuleKind.NodeNext, moduleResolution: ts.ModuleResolutionKind.NodeNext, skipLibCheck: true });
  const checker = program.getTypeChecker(); const result = new Map<string, Set<string>>();
  for (let index = 0; index < SDK_PACKAGES.length; index += 1) {
    const source = program.getSourceFile(entrypoints[index]!); const symbol = source && checker.getSymbolAtLocation(source);
    result.set(SDK_PACKAGES[index]!, new Set(symbol ? checker.getExportsOfModule(symbol).map((item) => item.name) : []));
  }
  return result;
}
const exportsByPackage = mode === "base" ? publicExports() : new Map<string, Set<string>>();
function validateTypeScriptImports(fence: Fence, source: string) {
  for (const match of source.matchAll(/import\s+(?:type\s+)?\{([^}]+)\}\s+from\s+["']@actuarial-ts\/(core|data|interchange|compliance|agents)["']/g)) {
    for (const item of match[1]!.split(",")) {
      const name = item.trim().replace(/^type\s+/, "").split(/\s+as\s+/)[0]!.trim();
      if (name && !exportsByPackage.get(match[2]!)?.has(name)) failures.push(`${fence.path} fence ${fence.ordinal}: ${name} is not a public @actuarial-ts/${match[2]} export`);
    }
  }
}
function checkBaseFence(fence: Fence, source: string) {
  ...
  if (["ts", "typescript"].includes(language)) {
    const output = ts.transpileModule(source, { reportDiagnostics: true, compilerOptions: { target: ts.ScriptTarget.ES2022, module: ts.ModuleKind.NodeNext, moduleResolution: ts.ModuleResolutionKind.NodeNext } });
    for (const diagnostic of output.diagnostics ?? []) if (diagnostic.category === ts.DiagnosticCategory.Error) failures.push(`${fence.path} fence ${fence.ordinal}: TypeScript syntax failed (${diagnostic.code})`);
    validateTypeScriptImports(fence, source);
  } else if (["js", "javascript"].includes(language)) {
    const checked = spawnSync(process.execPath, ["--check", "-"], { cwd: root, input: source, encoding: "utf8" }); if (checked.status !== 0) failures.push(`${fence.path} fence ${fence.ordinal}: JavaScript syntax failed`);
  } else if (language === "json") {
    try { JSON.parse(source); } catch { failures.push(`${fence.path} fence ${fence.ordinal}: strict JSON parse failed`); }
  } else if (language === "jsonc") {
    const parsed = ts.parseConfigFileTextToJson(fence.path, source); if (parsed.error) failures.push(`${fence.path} fence ${fence.ordinal}: JSONC parse failed`);
  } else if (["bash", "sh"].includes(language)) {
    const checked = spawnSync("bash", ["-n"], { cwd: root, input: source, encoding: "utf8" }); if (checked.status !== 0) failures.push(`${fence.path} fence ${fence.ordinal}: shell syntax failed`);
  } else if (["yaml", "yml"].includes(language)) {
    try { const result = parseDocument(source); if (result.errors.length) throw result.errors[0]; } catch { failures.push(`${fence.path} fence ${fence.ordinal}: YAML parse failed`); }
  }
}
